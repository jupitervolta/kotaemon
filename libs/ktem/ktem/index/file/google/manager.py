...
from jvis.google.manager import GoogleManager as _GoogleManager
...
class GoogleManager:
    def __init__(self, app):
        self._app = app
        self._google_manager = _GoogleManager()

        indexes = [
            index for index in self._app.index_manager.indices
            if index.name == "Google Collection"
        ]
        if len(indexes) != 1:
            raise ValueError("Google Collection index not found")
        self._index = indexes[0]

    def sync(self):
        print("Syncing Google...")
        settings = self._app.default_settings.flatten()
        for i, (result, info) in enumerate(self.sync_with_google(settings)):
            print(f"{i} result: {result}")
            print(f"{i} info: {info}")
        print("Syncing Google completed")

    def sync_with_google(self, settings) -> Generator[tuple[str, str], None, None]:
        """Sync with Google"""
        index_fn = lambda file: self.index_fn(file, True, settings)
        yield from self._google_manager.sync(engine, index_fn)
