...
import gradio as gr
from jvis.google.db import GoogleSyncs
from jvis.google.manager import GoogleManager
...
class GoogleIndexPage(FileIndexPage):
    def __init__(self, app, index):
        ...
        self._index = index
        self._google_manager = GoogleManager()
                ...
                with gr.Column() as self.upload:
                    with gr.Tab("Actions"):
                        ...
                        self.sync_button = gr.Button(
                            "Sync with Google", variant="primary"
                        )
    ...
    def sync_with_google(self, settings):
        """Sync with Google"""
        gr.Info(f"Starting synchronization with Google...")

        index_fn = lambda file: self.index_fn(file, True, settings)
        cum_result, cum_info = [], []
        _iter = self._google_manager.sync(engine, index_fn)
